window.function = function (data, overlayDataJson, width, height, type) {
  ...
  // --- Input Handling & Cleaning (Still need to ensure basic JSON validity) ---
  ...
  try {
    // Basic cleaning: remove known invalid JS code, quote keys
    ...
    // Quote keys: handles keys at start of object or after a comma
    tempString = tempString.replace(/([{,]\s*)([a-zA-Z0-9_]+)(\s*:)/g, '$1"$2"$3');
    // Ensure wrapped in brackets if not already
    if (tempString && !tempString.startsWith('[')) { tempString = '[' + tempString; }
    if (tempString && !tempString.endsWith(']')) { tempString = tempString + ']'; }

    // Final sanity check and parse test
    if (!tempString || tempString === "[]" || tempString === "") { cleanedDataString = '[]'; }
    else { JSON.parse(tempString); cleanedDataString = tempString; console.log("DEBUG: Primary data string cleaned."); }
...
am5.ready(function() {
  ...
  // --- Configuration & Data ---
  const primaryDataString = ${JSON.stringify(cleanedDataString)}; // Use cleaned string
  ...
  // --- NO Time Parsing Helper Function Needed ---

  // --- Data Parsing Function ---
  // *** SIMPLIFIED: No sorting, no timestamps added ***
  function parseChartData(primaryStr, overlayStr) {
    let primaryData = [];
    let parsedOverlayData = null;
    let hasValidOverlay = false;

    // Parse Primary Data (JSON is assumed clean now)
    try {
      let rawPrimary = JSON.parse(primaryStr);
      if (Array.isArray(rawPrimary)) {
        // Filter for required structure ONLY
        primaryData = rawPrimary.filter(item =>
            item && typeof item === 'object' &&
            item.hasOwnProperty('time') && typeof item.time === 'string' && // Expect string time
            item.hasOwnProperty('value') && typeof item.value === 'number'   // Expect number value
        );
        console.log("Primary data parsed & filtered:", primaryData.length);
        if (primaryData.length !== rawPrimary.length) {
             console.warn("Some primary data items were filtered out (missing time/value or wrong types).");
        }
      } else { console.warn("Parsed primary data is not an array."); }
    } catch (e) { console.error("Error parsing primary data JSON:", e); primaryData = []; } // Should be rare now

    // Parse Overlay Data (simple filter)
    try {
            ...
            for (const key in rawOverlay) {
                if (Object.hasOwnProperty.call(rawOverlay, key)) {
                    ...
                    if(Array.isArray(weekDataRaw)) {
                        ...
                        else { console.warn("Overlay data for key \\"" + key + "\\" had no valid items after filtering."); }
  ...
  // *** SIMPLIFIED: Extracts unique categories IN ORDER OF APPEARANCE from primary data ***
  // *** ASSUMES primaryData is ALREADY chronologically sorted ***
  function prepareAxisCategories(primaryData) {
      ...
      try {
          ...
          // Filter for uniqueness while preserving order of first appearance
          let uniqueCategoryStrings = categoryStrings.filter((value, index, self) => self.indexOf(value) === index);
          // Map to the format AmCharts axis expects { categoryFieldName: categoryValue }
          let xAxisData = uniqueCategoryStrings.map(timeStr => ({ time: timeStr })); // Using "time" as categoryField
          console.log("Axis categories prepared from primary data (preserving order):", xAxisData.length);
          return xAxisData;
      } catch (e) {
  ...
  function createChartAndAxes(root, xAxisData) {
    ...
    var xRenderer = am5xy.AxisRendererX.new(root, { minGridDistance: 70 }); xRenderer.labels.template.setAll({ fontSize: 8, rotation: -90, centerY: am5.p50, centerX: am5.p100, paddingRight: 5 });
    ...
    var yRenderer = am5xy.AxisRendererY.new(root, {}); var yAxis = chart.yAxes.push(am5xy.ValueAxis.new(root, { maxPrecision: 2, renderer: yRenderer }));
    console.log("Chart and axes created."); return { chart, xAxis, yAxis };
   }


  // --- Primary Series Creation (Using CategoryAxis - Unchanged) ---
  function createPrimarySeries(chart, root, primaryData, xAxis, yAxis) {
    console.log("Creating primary series...");
    var areaSeries = chart.series.push(am5xy.LineSeries.new(root, { name: "Selected Week", xAxis: xAxis, yAxis: yAxis, valueYField: "value", categoryXField: "time", fillOpacity: 0.5, connect: false, tooltip: am5.Tooltip.new(root, { labelText: "{valueY.formatNumber('#.00')} @ {categoryX}" }) })); areaSeries.strokes.template.set("strokeWidth", 2); areaSeries.fills.template.setAll({ visible: true, fill: areaSeries.get("stroke") }); areaSeries.bullets.push(function() { return am5.Bullet.new(root, { sprite: am5.Circle.new(root, { radius: 3, fill: areaSeries.get("stroke") }) }); });
    areaSeries.data.setAll(primaryData); areaSeries.appear(1000);
    var columnSeries = chart.series.push(am5xy.ColumnSeries.new(root, { name: "Selected Week (Interval)", xAxis: xAxis, yAxis: yAxis, valueYField: "value", categoryXField: "time", connect: false, tooltip: am5.Tooltip.new(root, { labelText: "Interval: {valueY.formatNumber('#.00')} @ {categoryX}" }) })); columnSeries.columns.template.adapters.add("fill", function(fill, target) { return (target.dataItem && target.dataItem.get("valueY") < 0) ? am5.color(0x8B0000) : fill; }); columnSeries.columns.template.adapters.add("stroke", function(stroke, target) { return (target.dataItem && target.dataItem.get("valueY") < 0) ? am5.color(0x8B0000) : stroke; }); columnSeries.columns.template.setAll({ strokeWidth: 1, width: am5.percent(60) });
    columnSeries.data.setAll(primaryData); columnSeries.hide(0); columnSeries.appear(1000);
    console.log("Primary series created."); return [areaSeries, columnSeries];
   }


  // --- Overlay Series Creation (Using CategoryAxis - Unchanged) ---
  ...
  // *** Create axis categories directly from (assumed sorted) primary data ***
  const xAxisData = prepareAxisCategories(primaryData);
  const { chart, xAxis, yAxis } = createChartAndAxes(root, xAxisData); // Create axes using these categories
