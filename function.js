...
am5.ready(function() {
  ...
  function parseTimeStringToTimestamp(timeString) {
      ...
      const originalTrimmed = timeString.trim(); // For logging
      // Try slightly more flexible regex
      const parts = originalTrimmed.match(/(\w+)\s+(\d{1,2}):(\d{2})\s*(AM|PM)/i);
      if (!parts) { console.warn("DEBUG parseTime: Regex failed for string:", originalTrimmed); return null; }
      ...
      if (!dayOffset) { console.warn("DEBUG parseTime: Invalid day name:", dayStr, "in string:", originalTrimmed); return null; }
      if (isNaN(hour) || isNaN(minute)) { console.warn("DEBUG parseTime: Invalid hour/minute:", parts[2], parts[3], "in string:", originalTrimmed); return null; }
      if (ampm === "PM" && hour < 12) hour += 12;
      if (ampm === "AM" && hour === 12) hour = 0;
      const date = new Date(Date.UTC(2024, 0, dayOffset, hour, minute));
      if (isNaN(date.getTime())) { console.warn("DEBUG parseTime: Failed to create valid Date object for:", originalTrimmed); return null; }
      return date.getTime();
  }
        ...
        primaryData = rawPrimary
          .map(item => {
              if (item && typeof item === 'object' && item.hasOwnProperty('time') && item.hasOwnProperty('value') && typeof item.value === 'number') {
                  ...
                  return timestamp !== null ? { ...item, _tempTimestamp: timestamp } : null;
              } return null; })
          .filter(item => item !== null);
        primaryData.sort((a, b) => a._tempTimestamp - b._tempTimestamp);
        console.log("Primary data parsed & sorted:", primaryData.length);
                ...
                if (Object.hasOwnProperty.call(rawOverlay, key)) {
                    ...
                    if(Array.isArray(weekDataRaw)) {
                         ...
                         const processedWeekData = weekDataRaw.filter((item, index) => {
                             ...
                             // *** ADDED DEBUGGING FOR OVERLAY FILTER ***
                             if (!isValid && invalidItemCount < 5) { // Log first few failures
                                 console.warn("DEBUG Overlay Filter Fail:", "Key:", key, "Index:", index, "Item:", JSON.stringify(item));
                                 invalidItemCount++;
                             }
                             return isValid;
                         });
                        ...
                        if (processedWeekData.length > 0) {
                            ...
                            if (invalidItemCount > 0) {
                                console.warn("Overlay key", key, "had", invalidItemCount, "invalid items filtered out (logged first 5).");
                            }
                        } else {
                             // This log now means *all* items failed the filter
  ...
  function prepareAxisCategories(sortedPrimaryData) { /* ... */
