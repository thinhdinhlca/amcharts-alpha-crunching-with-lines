window.function = function (data, overlayDataJson, width, height, type) {
  ...
  let chartHeight = height.value ?? 550; // Pixels
  let chartTypeLabel = type.value ?? "Value"; // Default label

  // --- HTML Template ---
  // Use standard quotes or concatenation for strings inside the script block
  let ht = `
<!DOCTYPE html>
<html>
<head>
    ...
    try {
      ...
      if (Array.isArray(rawPrimary)) {
        ...
        if (primaryData.length !== rawPrimary.length) {
            console.warn("Some primary data items filtered out (missing time/value).");
        }
      } else { console.warn("Primary data is not an array."); }
    } catch (e) { console.error("Error parsing primary data:", e); }
                ...
                if (Object.hasOwnProperty.call(parsedOverlayData, key)) {
                    ...
                    if(Array.isArray(weekData) && weekData.every(item => item && typeof item === 'object' && item.hasOwnProperty('time') && item.hasOwnProperty('value'))) {
                        validKeys++;
                         ...
                         // *** FIXED: Use concatenation or standard quotes ***
      ...
      for (const weekKey in overlayData) {
        if (Object.hasOwnProperty.call(overlayData, weekKey)) {
          ...
          var lineSeries = chart.series.push(am5xy.LineSeries.new(root, {
            ...
            // *** FIXED: Use standard quotes for AmCharts labelText ***
