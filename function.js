...
<head>
  ...
  <style>
    ...
    /* Attempt to force tooltip text color (might be overridden by amCharts inline styles) */
...
am5.ready(function() {
  ...
  const whiteColor = am5.color(0xffffff); // Define white color constant
  ...
  // --- Data Parsing Function --- (Using basic console.log as per last fix)
  ...
  function prepareAxisCategories(primaryData) {
     ...
     if (!primaryData || primaryData.length === 0) {
        console.log("WARNING: Primary data is empty for axis prep, axis will be empty.");
        return [];
      }
      try {
  ...
  // *** ADDED: yAxis.set("forceZero", true); ***
  function createChartAndAxes(root, xAxisData) {
    ...
    if (xAxisData && xAxisData.length > 0) {
        ...
        console.log("WARNING: X-Axis has no data categories to set.");
    }
    ...
    // *** Ensure the Y axis includes zero, which helps with zero-based fills ***
  ...
  // *** MODIFIED: Ensured tooltip labelTextColor for valueAreaSeries ***
  function createPrimarySeries(chart, root, primaryData, xAxis, yAxis) {
    ...
    valueAreaSeries = chart.series.push(am5xy.LineSeries.new(root, {
      ...
      valueYField: "value",
      categoryXField: "time",
      openValueYField: "valueOpen", // Fill baseline = 0
      ...
      tooltip: am5.Tooltip.new(root, {
          ...
          getFillFromSprite: false, // Do not inherit fill color for background
          getStrokeFromSprite: true, // Inherit stroke color for pointer
          // *** Ensure white text color ***
          labelTextColor: whiteColor,
          background: am5.RoundedRectangle.new(root, {
              fill: am5.color(primaryOutlineColor), // Use outline color for bg
              fillOpacity: 0.9
          }),
    ...
    console.log("Value Area series created. Fill opacity:", valueAreaSeries.get("fillOpacity"), "Fill color:", valueAreaSeries.get("fill")); // Log fill properties
    ...
    value2Series = chart.series.push(am5xy.ColumnSeries.new(root, {
      ...
      tooltip: am5.Tooltip.new(root, {
          ...
          labelTextColor: whiteColor, // Use white text here too
          fontSize: tooltipFontSize,
  ...
  // *** MODIFIED: Added background fill and labelTextColor to overlay tooltips ***
     ...
     try {
       for (const weekKey in overlayData) {
         if (Object.hasOwnProperty.call(overlayData, weekKey)) {
           ...
           const seriesColor = am5.color(colors[weekKey] || root.interfaceColors.get("grid")); // Determine color first
           ...
           var lineSeries = chart.series.push(am5xy.LineSeries.new(root, {
               ...
               valueYField: "value",
               categoryXField: "time",
               stroke: seriesColor, // Use the determined color
               connect: false,
               tooltip: am5.Tooltip.new(root, {
                 pointerOrientation: "horizontal",
                 getStrokeFromSprite: true, // Pointer color matches line
                 // *** Ensure white text & set background based on series color ***
                 labelTextColor: whiteColor,
                 background: am5.RoundedRectangle.new(root, {
                    fill: seriesColor, // Use the series color for the background
                    fillOpacity: 0.9 // Make it slightly opaque
                 }),
                 fontSize: tooltipFontSize,
  ...
  if (!primaryData || primaryData.length === 0) {
    console.log("WARNING: No valid primary data to display after parsing. Chart generation stopped.");
    try {
